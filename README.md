...
3. 検索欄にタグを入れると候補が出ます（英語のタグ名でも「ロングヘア」「ろんぐへあ」の
   ような日本語でも引けます）。`hair` で `long_hair` が出るように**部分一致**で引け、
   前方一致を先に、部分一致をその後に、それぞれ Danbooru の投稿数が多い順で並びます。
   候補をエリアへドラッグするか、クリックで適用エリアの末尾に追加します。
...
6. **セーブ**欄に名前を入れて「保存」を押すと、両エリアの中身をまるごと残せます。
   一覧のチップをクリックすればその状態に戻り、右クリックで消せます（後述）。
7. エリアのヘッダーの「クリア」で、そのエリアを空にして作り直せます。押し間違えても
   `Ctrl+Z` で戻せます。
...
ユーザーが手で直した対訳、名前を付けて保存したグループ（プリセット）、
名前を付けて保存したタグ構成（セーブ）は ComfyUI の userdata に入ります。

- `data/user/default/danbooru-tag-composer/overrides.json`
- `data/user/default/danbooru-tag-composer/presets.json`
- `data/user/default/danbooru-tag-composer/saves/<セーブ名>.json`
...
ツリーはワークフローと一緒に保存されますが、**ワークフローとは別にタグ構成だけを
残したい**ことがあります。同じタグの組を別のワークフローで使い回したい、失敗しそうな
編集の前に控えを取っておきたい、といった場合です。サイドバーの**セーブ**欄がそれで、
プリセットが「グループ 1 つ」を残すのに対し、こちらは**適用エリアと非適用エリアの中身を
まるごと**残します。ロードすると、どのタグがどちらのエリアにあったか・グループの構造・
重み・対訳・グループの開閉まで、保存した瞬間とそのままの状態に戻ります。

置き場所は 2 つあり、どちらも中身は同じ形式です。

- **保存 / 一覧のクリック** — ComfyUI の userdata（`saves/` 以下）に 1 件 1 ファイルで
  置きます。一覧は新しい順に並び、右クリックで削除できます。
- **書き出し / 取り込み** — 手元の `.json` ファイルとして出し入れします。控えを取ったり、
  ワークフローを渡さずにタグ構成だけを人に渡したりするのに使います。

読み込むと現在の内容は置き換わりますが、書き換わるのはノードのウィジェットだけなので
間違えても `Ctrl+Z` で戻せます。中身が空でないときは確認を挟みます。
...
`tree` はノードの `tags` ウィジェットに入るものと**同じ形をそのまま**入れています。
別の形に詰め替えると、ツリーの形を変えたときに直す場所が 2 つに増えるためです。
`version` はこの封筒の版で、`tree.version` とは別物です。

`options` のキーはノードのウィジェット名（`snake_case`）に揃えてあります。ファイルを
見ながらノードの設定を手で合わせられるほうが役に立つためです。ロード時にはここも
復元するので、出力される文字列まで同じに戻ります。`options` を持たないファイルも
受け付け、そのときはノードの現在の設定に手を付けません。

読み込み側は緩めに作ってあり、封筒に入っていない**ツリーそのもの**（ノードの `tags`
ウィジェットの値をコピーしてきたもの）も受け付けます。逆にワークフロー JSON のような
別物は `active` も `inactive` も持たないので弾かれ、空のツリーで上書きされることは
ありません。

この形式を読み書きするのはブラウザ側（`web/js/save.js`）だけです。ワークフローの実行に
要るのはノードの `tags` ウィジェットの値だけなので、バックエンドはセーブファイルを
知りません。
